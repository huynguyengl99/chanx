...
def test_generated_client_resubscribes_every_handle(
    generated: Path, tmp_path: Path
) -> None:
    """After a reconnect a client replays its subscriptions from one place."""
    script = (
        f"import sys, asyncio; sys.path.insert(0, {str(tmp_path)!r})\n"
        "from client.gen_hub.client import GenHubClient\n"
        "c = GenHubClient('localhost:8000')\n"
        "sent = []\n"
        "c.send_json = lambda data: sent.append(data) or asyncio.sleep(0)\n"
        "c.gen_discussion_topic(pk=5)\n"
        "c.gen_discussion_topic(pk=6)\n"
        "async def main():\n"
        "    task = asyncio.ensure_future(c.resubscribe())\n"
        "    await asyncio.sleep(0.05)\n"
        "    task.cancel()\n"
        "asyncio.run(main())\n"
        "print([(f['topic'], f['action']) for f in sent])\n"
    )
    result = subprocess.run(
        [sys.executable, "-c", script], capture_output=True, text=True, check=False
    )
    ...
    assert result.returncode == 0, result.stderr
    assert "('discussion:5', 'subscribe')" in result.stdout
